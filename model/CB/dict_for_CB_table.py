...
bn = df['beer_name'].tolist()
bn = map(lambda x: x.decode('utf-8', 'ignore').encode('ascii', 'ignore'), bn)
bn = map(lambda x: re.sub('\?\?', '', x).strip(), bn)
for n, i in enumerate(bn):
    if i == 'Fish Tale 10_ Anniversary Ale':
        bn[n] = 'Fish Tale 10 Anniversary Ale'
bn = map(lambda x: re.sub('\s+', ' ', x).strip(), bn)
df['beer_name'] = bn

df2 = df.groupby(['beer_name', 'state'], as_index=False).agg({'overall': 'mean', 'user_rating': 'mean'})

dict = {}
for i in range(1269):
    dict[df2.iloc[i, 0]] = [df2.iloc[i, 1], round(df2.iloc[i, 2]*4, 1), round(df2.iloc[i, 3], 1),
                            'https://www.google.com/#q='+str(df2.iloc[i, 0])]
